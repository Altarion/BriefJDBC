...
public class MenuBDDExo9 {
	...
	//Connection connecte = ConnectionBDD.connectInstance();
	...
	protected static Date inputRecruitDate(final Connection connect) throws InputMismatchException {
		Date recruit = null;
		try {
			Scanner in = new Scanner(System.in);
	...
	protected static void changeEmp(final Connection connect, final String lastname, final String firstname) {
		try {
			...
			switch (userChoice) {
			case 1:
				EmpModifierExo9.changeNoemp(connect, lastname, firstname);
				break;
			case 2:
				EmpModifierExo9.changeLastname(connect, lastname, firstname);
				break;
			case 3:
				EmpModifierExo9.changeFirstname(connect, lastname, firstname);
				break;
			case 4:
				EmpModifierExo9.changeJob(connect, lastname, firstname);
				break;
			case 5:
				EmpModifierExo9.changeSup(connect, lastname, firstname);
				break;
			case 6:
				EmpModifierExo9.changeRecruitdate(connect, lastname, firstname);
				break;
			case 7:
				EmpModifierExo9.changeSal(connect, lastname, firstname);
				break;
			case 8:
				EmpModifierExo9.changeComm(connect, lastname, firstname);
				break;
			case 9:
				EmpModifierExo9.changeNoserv(connect, lastname, firstname);
				break;
			case 10:
				menu(connect);
				break;
	...
	private static void selEmp(final Connection connect) throws InputMismatchException, SQLException {
		...
		String query = "SELECT nom, prenom FROM emp WHERE nom = '" + lastname + "' AND prenom = '" + firstname + "'";
		ResultSet selEmp = ConnectionBDD.query(connect, query);
		if (selEmp.isBeforeFirst()) {
			changeEmp(connect, lastname, firstname);
		} else {
	...
	private static void empDetail(final Connection connect) throws SQLException {
		...
		ResultSet query = ConnectionBDD.query(connect,
				"SELECT * FROM emp WHERE nom = '" + lastname + "' AND prenom = '" + firstname + "'");
		if (query.isBeforeFirst()) {
			while (query.next()) {
				StringBuilder result = new StringBuilder();
				result.append("N° Employé : ").append(query.getInt("noemp")).append(", Nom : ")
						.append(query.getString("nom")).append(", Prénom : ").append(query.getString("prenom"))
						.append(", Poste : ").append(query.getString("emploi")).append(", N° Supérieur : ")
						.append(query.getInt("sup")).append(", Embauche : ").append(query.getDate("embauche"))
						.append(", Salaire : ").append(query.getInt("sal")).append(", Commission : ")
						.append(query.getInt("comm")).append(", N° Service : ").append(query.getInt("noserv"));
				System.out.println(result);
			}
			menu(connect);
		} else {
	...
	private static void newEmp(final Connection connect) throws SQLException {
		...
		Date embauche = inputRecruitDate(connect);
		int sal = inputSal(connect);
		int comm = inputComm(connect);
		int noserv = inputNoserv(connect);
		String query = "INSERT INTO emp (noemp, nom, prenom, emploi, sup, embauche, sal, comm, noserv) VALUES ('"
				+ noemp + "', '" + lastname + "', '" + firstname + "', '" + emploi + "', '" + sup + "', '" + embauche
				+ "', '" + sal + "', '" + comm + "', '" + noserv + "')";
	...
	private static void delEmp(final Connection connect) throws SQLException {
		String lastname = inputLastname(connect).toUpperCase();
		String firstname = inputFirstname(connect).toUpperCase();
		String query = "DELETE FROM emp WHERE nom = '" + lastname + "' AND prenom = '" + firstname + "'";
		ConnectionBDD.Update(connect, query);
		StringBuilder result = new StringBuilder();
